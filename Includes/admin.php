<?php 
    ...
    function retrieve_bill_data($offset, $rowsperpage){
        global $con;
        $query  = "SELECT curdate() AS bdate , adddate( curdate(),INTERVAL 30 DAY ) AS ddate , user.id AS uid , user.name AS uname,user.usertype AS usertype,user.utility AS utility FROM user ";
        $query .= " LIMIT {$offset}, {$rowsperpage} ";
        // echo $query;
        $result = mysqli_query($con,$query);
        if($result === FALSE) {
    ...
    {
        global $con;
        $query  = "SELECT application.id AS id , application.application AS application , application.status AS status , user.name AS uname ";
        $query .= "FROM user , application ";
        $query .= "WHERE application.uid=user.id AND status='NOT PROCESSED' ";
        $query .= "ORDER BY application.id desc  ";
        $query .= "LIMIT {$offset}, {$rowsperpage} ";
    ...
    function retrieve_admin_stats($id)
    {
        ...
        $query3  = " SELECT count(id) AS unprocessed_complaints from application where status='NOT PROCESSED' ";
        // echo $query;
